...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    my_digit = player_score % 10
    oppenent_digit = opponent_score // 10 % 10
    result = 3 * abs(my_digit - oppenent_digit)
    if result > 1:
        return result
    else:
        return 1
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    """Return the points scored (就是返回得分)on a turn rolling NUM_ROLLS dice when the
