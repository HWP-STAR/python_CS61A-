...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def para_has_subject(para):
        para=split(remove_punctuation(lower(para)))
        for word in para:
            if word in subject:
                return True
        return False
    return para_has_subject
    # END PROBLEM 2
...
def furry_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
  # 基本情况1：如果已经超过 limit，立即返回
    if limit < 0:
        return float('inf')  # 或任何 > limit 的数
    
    # 基本情况2：如果某个字符串为空，剩余长度差就是需要的更改数
    if not typed or not source:
        return len(typed) + len(source)
    
    # 基本情况3：如果第一个字符相同，不需要替换
    if typed[0] == source[0]:
        return furry_fixes(typed[1:], source[1:], limit)
    
    # 基本情况4：如果第一个字符不同，需要1次替换
    # 注意 limit 减1，因为我们已经用掉了一次替换的机会
